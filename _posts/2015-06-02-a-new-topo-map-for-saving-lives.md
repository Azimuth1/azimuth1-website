...
[^2]: There aren't any up to date sources of the high precision contour lines available to our knowledge, so we haven't been able to replace the contour lines.  The new WorldDEM high resolution elevation data for the globe from Airbus that is coming available over the next few years might make the high resolution contours possible again.
